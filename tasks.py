...
class Task1_OOM:
    ...
    correct_root_cause   = "oom"   # keyword grader checks for

    NOISE_SERVICES = ["analytics-service", "notification-service", "inventory-service"]

    def setup(self, sim: ServiceSimulator) -> None:
        sim.set_oom("payments-service")
        # Inject noise into distractors (but keep them technically healthy)
    ...
    def grade(self, result: EpisodeResult) -> float:
        ...
        Scoring breakdown (sums to 1.0):
          0.40  root cause correctly identified
          0.30  correct service remediated (not a distractor)
          0.20  time efficiency  (≤8 steps = full marks, linear decay after)
          0.10  blast radius     (zero wrong restarts = full marks)
        """
...
class Task2_BadDeploy:
    ...
    difficulty  = "medium"
    description = (
        "A bad deploy of auth-service is causing cascading 503 errors across "
        "checkout-service, payments-service, and api-gateway. "
        "Six alerts are firing. Identify the root cause by tracing the dependency graph "
        "and roll back the correct service. Restarting downstream services is a trap — "
        "they will only recover after auth-service is fixed."
    )
    max_steps            = 25
    time_limit_minutes   = 60
    correct_service      = "auth-service"
    correct_action       = "rollback_deploy"
    correct_root_cause   = "deploy"  # keyword grader checks for

    NOISE_SERVICES = ["analytics-service", "data-pipeline-service",
    ...
    def grade(self, result: EpisodeResult) -> float:
        """
        Scoring breakdown:
          0.35  root cause correctly identified (bad deploy on auth-service)
          0.25  correct remediation (rollback, not restart)
          0.20  did NOT restart downstream services first (blast radius clean)
          0.15  time efficiency (≤14 steps = full marks)
          0.05  topology consulted before acting (tracked in notes)
        """
...
# ─────────────────────────────────────────────────────────────────────────────
# Task 3 — The Phantom Slowdown  (Hard)
# ─────────────────────────────────────────────────────────────────────────────
...
class Task3_PhantomSlowdown:
    ...
    description = (
        ...
        "There are no crashes, no obvious errors — only intermittent slowness. "
        "Root cause: data-pipeline-service runs a heavy cron job that table-locks "
        "a shared PostgreSQL table, starving checkout-service queries. "
        "Eight distractor alerts are firing. You must correlate time-series patterns "
        "across services and identify the cron schedule. "
        "Correct fix: scale up data-pipeline-service to reduce lock duration, "
        "OR page the team with the correct root cause."
    )
    max_steps            = 40
    time_limit_minutes   = 120
    correct_service      = "data-pipeline-service"
    correct_action       = "scale_up"          # or page_team with correct message
    correct_root_cause   = "cron"              # keyword grader
    CRON_PERIOD          = 15                  # minutes between spikes
    ...
    def setup(self, sim: ServiceSimulator) -> None:
        # Initial state: checkout is healthy (we're between spikes)
        sim.add_noise_to_service("analytics-service")
        sim.add_noise_to_service("notification-service")
        # data-pipeline looks slightly elevated — a subtle hint
        sim.services["data-pipeline-service"]["cpu"] = 48.0

    def initial_alerts(self) -> List[Alert]:
        return [
            # Real alerts (intermittent)
    ...
    def grade(self, result: EpisodeResult) -> float:
        """
        Hard task — partial credit for partial investigation.

        0.30  root cause identified (data-pipeline cron / DB lock)
        0.25  correct service targeted in remediation
        0.20  postmortem quality (explains periodic pattern)
        0.15  time efficiency (≤25 steps = full, decay above)
        0.10  blast radius (no wrong remediations)
        """
